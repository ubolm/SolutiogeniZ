...
export function ConversationInboxPanel({
  ...
  return (
    <section className="grid gap-5">
      <section className="overflow-hidden rounded-[1.7rem] border border-[#dfe5fb] bg-[linear-gradient(135deg,#ffffff_0%,#f5f8ff_50%,#ebf1ff_100%)] p-4 shadow-soft md:p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary-strong">
              Centro de conversaciones
            </p>
            <h2 className="font-heading mt-2 text-2xl font-semibold text-ink md:text-[2rem]">
          ...
          <div className="rounded-[1.2rem] border border-white/70 bg-white/80 px-3.5 py-3 backdrop-blur">
      ...
      <section className="grid gap-4 xl:grid-cols-[0.8fr_1.2fr]">
        <article className="rounded-[1.7rem] border border-line bg-white p-4 shadow-soft md:p-5">
          ...
          <div className="mt-5 grid gap-3">
            {spotlightItems.length === 0 ? (
              <p className="rounded-2xl border border-dashed border-line bg-paper px-4 py-4 text-sm text-muted">
                No encontramos conversaciones con esos filtros.
              </p>
            ) : (
              spotlightItems.map((item) => (
                <article
                  className="rounded-[1.2rem] border border-[#e7ebf6] bg-[#f8faff] px-3.5 py-3.5"
                ...
                >
                  ...
                  <p className="mt-2 text-sm leading-5 text-muted">{item.reason}</p>
                  {item.lead ? (
                    <Link
        ...
        <section className="rounded-[1.7rem] border border-line bg-white p-4 shadow-soft md:p-5">
          ...
          <div className="mt-5 grid gap-3 lg:grid-cols-[1.1fr_0.45fr_0.45fr]">
          ...
          <div className="mt-5 grid gap-2.5">
            {filteredConversations.length === 0 ? (
              ...
              filteredConversations.map((item) => {
                ...
                return (
                  <article
                    className="rounded-[1.25rem] border border-[#e5e9f4] bg-[linear-gradient(180deg,#fbfcff_0%,#f6f8fc_100%)] px-3.5 py-3.5 shadow-[0_10px_24px_rgba(15,19,36,0.05)]"
                    key={conversation.id}
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                    ...
                    <div className="mt-3 rounded-[1rem] border border-white/80 bg-white px-3.5 py-3">
                    ...
                    <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_auto] lg:items-center">
                      <div className="rounded-[1rem] bg-[#f7f9fc] px-3 py-2.5 text-sm leading-5 text-muted">
...
function ConversationMetric({
  ...
  return (
    <article className="rounded-[1.1rem] bg-white/82 p-3 backdrop-blur">
...
function PriorityCard({
  ...
  return (
    <article className={`rounded-[1.3rem] border p-4 shadow-soft ${toneClass}`}>
      ...
      <p className="mt-2 text-sm leading-5 text-current/80">{description}</p>
    </article>
  );
}
